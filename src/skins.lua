-- ============================================================
-- Rivals Modular -- Skins
-- Skin changer + wrap changer
-- ============================================================
...
local function getSkinCases()
    ...
    local caseNames = {"Spooky Skin Case", "Skin Case 2", "Skin Case 3", "Skin Case", "Other", "Festive Skin Case"}
    for _, name in ipairs(caseNames) do
        local ok, case = pcall(function()
            return viewModels:FindFirstChild(name)
        end)
        if ok and case then
...
local function saveSettings()
    pcall(function()
        if writefile then
            writefile(SKINS_FILE, HttpService:JSONEncode({ selectedSkins = selectedSkins }))
        end
    end)
end
...
local function getWrapWeapons()
    ...
    local skip = {
        ["Spooky Skin Case"] = true, ["Skin Case 2"] = true,
        ["Skin Case"] = true, ["Other"] = true, ["Festive Skin Case"] = true
    }
...
local function getWrapList()
    ...
    local ok, wraps = pcall(function()
        return LocalPlayer.PlayerScripts.Assets.WrapTextures:GetChildren()
    end)
...
function Skins.Update(_dt)
    -- Skin changer is event-driven, no per-frame work needed
end
...
function Skins.Init(deps)
    ...
    if page then
        ...
        GUI.AddButton(page, "Reset Skins",
            function()
                for weaponName, _ in pairs(appliedSkins) do
                    restoreOriginal(weaponName)
                end
                appliedSkins = {}
                selectedSkins = {}
                saveSettings()
        ...
        GUI.AddDropdown(page, "Weapon",
            function()
                local list = {"none"}
                for _, w in ipairs(getWrapWeapons()) do
                    table.insert(list, w)
                end
                return list
            end,
        ...
        GUI.AddDropdown(page, "Wrap",
            function()
                local list = {"none"}
                for _, w in ipairs(getWrapList()) do
                    table.insert(list, w)
                end
                return list
            end,
