-- ============================================================
-- Rivals Modular -- ESP
-- Highlight, billboard, boxes, tracers, health, distance
-- ============================================================
...
-- ------------------------------------------------------------
-- Drawing-based ESP (boxes, names, health bars, distance)
-- ------------------------------------------------------------
...
local function hideAllDrawings()
    for _, drawings in pairs(boxDrawings) do
        pcall(function() drawings.box.Visible = false end)
        pcall(function() drawings.fill.Visible = false end)
    end
    for _, name in pairs(nameDrawings) do
        pcall(function() name.Visible = false end)
    end
    for _, health in pairs(healthDrawings) do
        pcall(function() health.bg.Visible = false end)
        pcall(function() health.bar.Visible = false end)
    end
    for _, dist in pairs(distDrawings) do
        pcall(function() dist.Visible = false end)
    end
end

-- Get 2D screen bounds of a character
local function getCharacterBounds(character)
    ...
    local rootPos, rootVisible = Utils.WorldToScreen(root.Position)
    local headPos, headVisible = Utils.WorldToScreen(head.Position + Vector3.new(0, 1.5, 0))

    if not rootVisible and not headVisible then return nil end
    ...
    -- Estimate width based on height
...
function ESP.Update(_dt)
        ...
        for _, player in ipairs(Utils.GetEnemies()) do
            ...
            if humanoid and humanoid.Health > 0 and root and head then
                ...
                if okRP and (rootPos - localRoot.Position).Magnitude <= maxDist then
                    local okHP, targetPos = pcall(function()
                        return head.Position or root.Position
                    end)
                    if okHP then
    ...
    for _, player in ipairs(Utils.GetEnemies()) do
        ...
        if Config.Get("ESP_Name") then
            ...
            local nameDraw = nameDrawings[player]
        ...
        if Config.Get("ESP_HealthBar") then
            ...
            if healthDraw then
                ...
                local healthColor
                if pct > 0.6 then
                    healthColor = Color3.fromRGB(0, 220, 80)
                elseif pct > 0.3 then
                    healthColor = Color3.fromRGB(255, 190, 0)
                else
                    healthColor = Color3.fromRGB(235, 45, 45)
                end
            ...
            local healthDraw = healthDrawings[player]
            if healthDraw then
                healthDraw.bg.Visible = false
                healthDraw.bar.Visible = false
            end
        end
        ...
        if Config.Get("ESP_Studs") then
            ...
            local distDraw = distDrawings[player]
            if distDraw then distDraw.Visible = false end
        end
    end

    -- Cleanup players no longer visible
...
-- ------------------------------------------------------------
-- Billboards + highlights (slow refresh)
-- ------------------------------------------------------------
...
function ESP.Cleanup()
    ...
    for player, drawings in pairs(boxDrawings) do
        Utils.DestroyDrawing(drawings.box)
        Utils.DestroyDrawing(drawings.fill)
        boxDrawings[player] = nil
    end
    for player, name in pairs(nameDrawings) do
        Utils.DestroyDrawing(name)
        nameDrawings[player] = nil
    end
    for player, health in pairs(healthDrawings) do
        Utils.DestroyDrawing(health.bg)
        Utils.DestroyDrawing(health.bar)
        healthDrawings[player] = nil
    end
    for player, dist in pairs(distDrawings) do
        Utils.DestroyDrawing(dist)
        distDrawings[player] = nil
    end
end
